# -*- coding: utf-8 -*-
"""
Created on Fri Apr  3 13:22:12 2026
...
# -*- coding: utf-8 -*-
"""
Experimento M4 para N=500 (script independiente)
Ejecuta el experimento con N=500 y guarda resultados.
...
def generate_graph(seed, N=500, alpha=1.5, lam=0.5, d=2):
    np.random.seed(seed)
...
# =========================================================
# EXPERIMENTO PRINCIPAL PARA N=500
# =========================================================

def run_experiment_N500(lam_values, N=500, seeds=3, n_solutions=15, max_steps=300):
    results = []
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("Experiment M4 for N=500")
    print("="*60)
    
    lam_values = np.linspace(0, 1, 10)
    N = 500  # <--- CAMBIA AQUÍ (200 → 500)
    seeds = 3
    n_solutions = 15
    max_steps = 300
    ...
    estimated = bench_time * total_opt / 100 * max_steps
    print(f"  Estimated per optimization: {bench_time:.2f}s")
    print(f"  Total estimated time: {estimated/60:.1f} minutes ({estimated/3600:.1f} hours)")
    
    if estimated > 600:  # más de 10 minutos preguntar
    ...
    results = run_experiment_N500(lam_values, N=N, seeds=seeds, 
                                  n_solutions=n_solutions, max_steps=max_steps)
    elapsed = time.time() - start_time
    
    # Guardar resultados
    os.makedirs("experiment_M4_N500", exist_ok=True)
    with open("experiment_M4_N500/results.json", "w") as f:
        json.dump(results, f, indent=2)
    
    print(f"\n✅ Experiment completed!")
    print(f"   Real time: {elapsed/60:.1f} minutes ({elapsed/3600:.2f} hours)")
    print(f"   Results saved in: experiment_M4_N500/results.json")